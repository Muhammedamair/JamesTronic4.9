-- C21 Wave 3 Fix: Normalize phone format comparison in redeem_quote_share_token
-- auth.users.phone may be stored without '+' prefix, so normalize both sides
...
DECLARE
  ...
  v_normalized_intended_phone text;
...
BEGIN
  ...
  IF v_tok.intended_phone_e164 IS NOT NULL THEN
    ...
    -- Normalize both phones: strip leading + and any whitespace for comparison
    -- This handles cases where auth.users stores without + but token was created with +
    v_normalized_user_phone := regexp_replace(coalesce(v_user_phone, ''), '^\+', '');
    v_normalized_intended_phone := regexp_replace(v_tok.intended_phone_e164, '^\+', '');
    
    -- Check if null or mismatch
    IF v_user_phone IS NULL OR v_normalized_user_phone <> v_normalized_intended_phone THEN
