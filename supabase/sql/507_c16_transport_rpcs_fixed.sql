...
BEGIN
  ...
  -- Hash the provided OTP code to compare with stored hash
  v_hashed_otp := encode(digest(p_otp_code || (SELECT COALESCE(otp_salt, gen_random_uuid()::TEXT) FROM public.transport_job_otps WHERE id = v_otp_record.id LIMIT 1), 'sha256'), 'hex');
